...
async fn setup() -> (cdp_mock::CdpMock, Browser, Page) {
    ...
    mock.clear_navigate_params().await;
    (mock, browser, page)
}
...
async fn unarmed_navigate_sends_no_timeout_key() {
    let (mock, _browser, page) = setup().await;
    let fut = page
    ...
    let captured = mock.navigate_params().await;
    assert!(
        !captured.is_empty(),
...
async fn armed_navigate_uses_the_page_navigate_method_string() {
    ...
    assert!(
        !mock.navigate_params().await.is_empty(),
...
async fn armed_navigate_preserves_every_standard_field() {
    ...
    let (plain_mock, plain_browser, plain_page) = setup().await;
    let fut = plain_page
        .navigate_http_future(base.clone())
        .expect("navigate future");
    let _ = timeout(Duration::from_secs(3), fut).await;
    let plain = plain_mock.navigate_params().await;
    drop(plain_browser);

    let (armed_mock, armed_browser, armed_page) = setup().await;
    let fut = armed_page
        .navigate_http_future_with_timeout(base, Duration::from_millis(1500))
        .expect("navigate future");
    let _ = timeout(Duration::from_secs(3), fut).await;
    let armed = armed_mock.navigate_params().await;
    drop(armed_browser);
